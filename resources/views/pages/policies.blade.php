...
@section('seo')
    <x-seo>
        <x-slot:title>Política de Privacidade</x-slot>
        <x-slot:description>Política de Privacidade</x-slot>
    </x-seo>
@endsection

@section('mainClass', 'site-policies')
@section('containerAroundClass', 'container container-page')

@section('content')
    <section>
        <article>
            <h2>Política de privacidade para <strong>{{ config('app.hostname') }}</strong></h2>
            ...
            <p>A garantia da confidencialidade dos dados pessoais dos usuários desse site é importante para o <strong>{{ config('app.hostname') }}</strong>.</p>
            <p>Todas as informações pessoais que usem o <strong>{{ config('app.hostname') }}</strong> serão tratadas em concordância com a Lei Nº 13.709, de 14 de Agosto de 2018.</p>
            ...
            <p>O uso do <strong>{{ config('app.hostname') }}</strong> pressupõe a aceitação deste acordo de privacidade. O site <strong>{{ config('app.hostname') }}</strong> reserva-se ao direito de alterar este acordo sem aviso prévio. Deste modo, recomendamos que consulte a nossa política de privacidade com regularidade de forma a estar sempre atualizado.</p>
            ...
            <p>Utilizamos cookies para armazenar informação, tais como as suas preferências pessoais quando visita o nosso website. Como para por exemplo guardar suas preferências de tema quando retornar ao site.</p>
            ...
            <p>O <strong>{{ config('app.hostname') }}</strong> possui conexões para outros sites, que podem conter informações úteis. A nossa política de privacidade não é aplicada a sites de terceiros, pelo que, caso visite outro site a partir do nosso deverá ler a politica de privacidade do mesmo.</p>
